...
**Update: the host release is now verified and partitioned.** Six-policy source data contains 54,059 rows. The committed, target-blind boundary prepares 9,106 additional research rows across advertising, legal advice, medical advice and illegal-activity promotion. It reserves 43,576 rows, including every eligible financial-advice and spoiler example and the former Private split. Historical exposure excludes 54 reserve rows; comment/support overlap excludes 1,323 research rows. The new research cohort has not produced a feature score, so the original candidate counts and completion estimate below are not increased merely because data became available. [Protocol and exact next deliverable](RELEASED_DATA.md).
...
The major implemented families now have measured additions/removals, leakage controls, retained counts, negative results and reproducibility artifacts. Diminishing returns are visible for broad handcrafted expansion, raw-coordinate selection, metadata, low-rank alternatives and these frozen NLI/instruction models. **Global exhaustion is not established.** High-value unresolved avenues are:

1. Execute the newly enabled four-policy research round, then evaluate a frozen candidate on the reserved policies and rows. The source-access blocker is resolved; independent confirmation itself remains unexecuted.
2. Extend duplicate/conflicting-label and approximate-copy analysis to the new corpus. Its 39 conflicting body/policy groups include 25 within the same subreddit and 14 across subreddits. These may reflect annotation noise or context differences and require documented sensitivity analysis. Conversation timestamps, threads and authors remain unavailable.
3. Policy-sensitive semantic errors, especially legal advice, evaluated against newly available rule-level confirmation data. The fixed instruction probe is now complete and negative. Its failure and the NLI result do not rule out larger or differently trained joint encoders; selecting those against the same two exposed rules would add selection bias.
