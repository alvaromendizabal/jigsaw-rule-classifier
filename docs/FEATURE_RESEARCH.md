...
The expanded study is now executed: **295 fixed fits** (238 primary, 22 changed-training sensitivities and 35 retrieval fits), plus six frozen embedding-resolution comparisons. It uses 11,135 development rows across four policies; 43,576 rows remain reserved. Notebook `02` now reports the broader screening, matched ablations, robustness and negative findings. [Current results](EXPANDED_STUDY.md).

**About 80% overall completion** is a planning estimate. The rubric credits data/validation 17/20, feature research 31/35, model/evaluation/inference 10/20, engineering 14/15 and presentation 8/10. Working references and diagnostic evaluation earn partial model credit; independent confirmation, final calibration and production promotion do not. Notebook `02` is approximately **90% complete in implementation and narrative**, with its scientific gate still open. These are effort-weighted judgments, not measured employer ratings.

The leading 1,024-dimensional frozen centroid has a substantial matched development gain (+0.2314 AUC), but it is not an independently confirmed final model. Most of that improvement comes from illegal-activity promotion; legal and medical advice remain weak. Familiar-policy success does not establish policy transfer.

Diminishing returns are now well supported for adding more generic handcrafted columns, this labeled-retrieval geometry, shorter Matryoshka prefixes, community/target shortcuts and the tested lexical/SVD controls. **Global feature exhaustion is not established.** The finite remaining feature milestone is:

1. Test the pinned model's asymmetric query/document formatting: retain the rule-conditioned comment query and compare plain support documents with the current rule-conditioned supports. There are 2,217 unique plain support texts, so this is a bounded cache extension rather than a new large-model search. Freeze the comparison before scoring.
2. Complete a structured development-only legal/medical error audit. Inspect requests versus assertions, personal versus generic advice, satire/quotation, and supplied-example inconsistency. Eight extreme-score examples already inspected suggest these mechanisms, but that deliberately selected sample cannot estimate error prevalence or establish label correctness. Use the audit to choose one justified direct policy-intent representation check; prior NLI/instruction failures on two policies do not rule out every formulation.
3. Record a stopping decision based on matched gains, per-policy behavior, probability losses, robustness and cost. Two of 11,973 unique cached queries were truncated at 256 tokens, so a blanket long-context rerun is lower priority than policy intent and input formatting. Do not consume the reserve to choose features.

After that gate, freeze a candidate/reference and probability-treatment protocol for **one reserved confirmation evaluation**. Then promote the accepted artifact into inference, measure offline parity and resource use, add calibrated/selective-review diagnostics and finish an example-driven demonstration with model/data cards. The old lexical notebook remains an explicitly named reference, not an accidentally outdated promoted model.

A 9.9/10 claim would require those acceptance results and an easy, reliable product demonstration. A count of features, passing tests, an attractive README or a development AUC alone cannot supply them. [Release milestones](ROADMAP.md#next-three-deliverables).
