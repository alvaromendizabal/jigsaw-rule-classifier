...
2. **A verified inference product.** Package the accepted benchmark representation for offline inference. Keep a separately identified original-training-only competition workflow; the fitted benchmark uses post-competition development labels. Demonstrate artifact lineage, feature parity, batch independence, resumability, missing-support behavior and named-hardware latency/memory measurements. Add a small example-driven interface and probability/triage diagnostics. No automatic Kaggle submission is part of this work.
