...
| 2 · Finish feature research | 210 controlled fits completed on two policies; 9,106 additional research rows now prepared for four-policy development | Frozen new folds; duplicate/conflicting-label sensitivity; matched feature additions/removals; stable selected representation and stopping rationale |
...
The host's post-competition release resolves the earlier lack of additional labeled policies. [RELEASED_DATA.md](RELEASED_DATA.md) locks the next boundary: original training plus 9,106 retained Public rows for four development policies, with 43,576 rows reserved. No new study score is claimed. The full source has 54,059 rows; 1,323 research rows are excluded for crossing the protected comment boundary and 54 reserved rows for historical exposure. The preparation code interprets only retained research targets.

## Next three deliverables

1. **A feature decision that survives broader development.** Use 11,135 available development rows before duplicate handling. Audit the 544 repeated body/policy rows and 39 conflicting-label groups; do not silently discard or reconcile them. Commit the executable study specification, repeat major-family screening/additions/removals within the expanded training partitions, compare the strongest existing representations on identical folds, and investigate further families only with a stated hypothesis. Extend exact/approximate-copy and support-context stress tests to the new cohort. Deliver executed notebook `02`, per-family evidence and one frozen candidate.
...
These are acceptance milestones, not a promise of a numerical employer rating. A four-policy development result and a two-policy reserve are substantially better evidence than repeated selection on two policies; they still do not establish performance on every possible community norm. Fine-tuning or a larger model is optional and needs a development-set justification after the representation work.
