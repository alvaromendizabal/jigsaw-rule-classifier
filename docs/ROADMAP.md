...
1. **A validated final model.** Implement the fixed training-policy-membership route with the validated familiar-policy feature model and unseen-policy centroid. Replay OOF behavior, develop calibration with another validation level, and freeze the exact candidate/reference and acceptance rules before one reserved comparison. Report familiar and unseen policies separately. The route is a proposal, not yet a measured composite result. [Detailed next milestone](FINAL_MODEL_PLAN.md).
