...
2. Fit candidate feature banks and model artifacts only from development data, including cross-fitted target-derived training features. Verify that unseen-policy rows bypass those banks completely. Record source, schema, screen, encoder, policy-set and calibration identities in the inference manifest.
