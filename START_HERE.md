...
**Project status:** the scoped research and local inference product are complete. **Submission notebook:** [`kaggle/submission.ipynb`](kaggle/submission.ipynb) · [download](https://github.com/alvaromendizabal/jigsaw-rule-classifier/raw/refs/heads/main/kaggle/submission.ipynb). The corrected notebook is also **[already run in Kaggle as Version 2](https://www.kaggle.com/code/alvaromendizabal/jigsaw-original-training-reference/output?scriptVersionId=348605174)**. The [delivery guide](docs/DELIVERY.md) gives the final submission clicks, current GitHub download and versioned AWS backups. Submission is ready but has not been sent; no Kaggle score is claimed.
...
**The notebook is already uploaded and successfully run.** Open [Version 2's output](https://www.kaggle.com/code/alvaromendizabal/jigsaw-original-training-reference/output?scriptVersionId=348605174), click **Submit to Competition**, select **Kaggle input path fix - GitHub 7e98f91** and the root **submission.csv**, then click **Submit**. Wait for **Complete** and a score on the competition's Submissions tab. The final submission click is still pending. You do not need to download anything from AWS or create another notebook. [Detailed steps and current status](docs/DELIVERY.md#your-kaggle-submission).

To recreate the run, import the current canonical notebook from GitHub, attach the official competition data, select CPU/no accelerator, disable Internet, and use **Save Version → Save & Run All**. Both the current `/kaggle/input/competitions/` mount and the older mount are supported. The old S3 `v1.0.0` notebook snapshot predates this path fix. The notebook produces `/kaggle/working/submission.csv`; its original 10-row preview is not the hidden-test output or a leaderboard score.
