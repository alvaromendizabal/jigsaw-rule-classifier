...
**Project status:** the support-adapted 4B method improves the matched novel-comment development comparison from 0.6146 to 0.7199 AUC. The approximately **0.92 Kaggle objective remains open**. [Measured rebuild](docs/COMPETITION_REBUILD.md).

**Current GPU notebook:** [kaggle/submission.ipynb](kaggle/submission.ipynb). It learns from the original training labels and legitimate supplied rule examples inside the offline Kaggle run. [Runtime and submission status](reports/checkpoints/kaggle_adaptation.json).

**Historical reference:** [kaggle/reference.ipynb](kaggle/reference.ipynb) preserves the CPU lexical method. Its saved [Kaggle Version 2](https://www.kaggle.com/code/alvaromendizabal/jigsaw-original-training-reference/output?scriptVersionId=348605174) scored **0.59191 public / 0.61956 private**. Those numbers do not describe the new neural notebook.
...
To recreate that historical run, import `kaggle/reference.ipynb`, attach the official competition data, select CPU/no accelerator, disable Internet, and use **Save Version → Save & Run All**. Both the current `/kaggle/input/competitions/` mount and the older mount are supported. The old S3 `v1.0.0` snapshot predates this path correction.

For the new neural candidate, import `kaggle/submission.ipynb`, attach the competition and `wowfattie/qwen3-4b-instruct-2507/transformers/default/1`, select **GPU T4 x2**, keep Internet off, and use **Save Version → Save & Run All**. The real preview and replay pass in the pinned 2026-06-30 environment; the saved version and hidden score are tracked in the [runtime receipt](reports/checkpoints/kaggle_adaptation.json). The notebook produces `/kaggle/working/submission.csv`; its ten-row preview verifies execution and is not the hidden-test output or a leaderboard score.
...
## Historical boundary: research and offline delivery complete

The feature-completion gate passed for the declared four-policy scope. The semantic-formatting/intent and fixed-fusion comparisons retained the original centroid. The final route used 9,263 screened features with accepted calibration for familiar policies, and the raw frozen centroid for unseen policies. Nested calibration rejected a harmful unseen-policy correction. The [confirmation protocol](docs/CONFIRMATION.md) and protected comparison accepted the fixed route: policy-macro AUC 0.7770 versus 0.6801, with improvements on all six policies. Its offline integration and verified restore are complete. The separately submitted lexical reference scored 0.61956 private. The present competition rebuild and its stronger neural notebook have their own original-label/support boundary and remain open until separately scored. [Completed historical release milestones](docs/ROADMAP.md#completed-deliverables).
