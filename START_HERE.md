...
**Project status:** the scoped research and local inference product are complete. **Submission notebook:** [`kaggle/submission.ipynb`](kaggle/submission.ipynb) · [download](https://github.com/alvaromendizabal/jigsaw-rule-classifier/raw/refs/heads/main/kaggle/submission.ipynb). An identical verified copy is in S3 at `releases/v1.0.0/kaggle/submission.ipynb`. The [delivery guide](docs/DELIVERY.md) gives the exact bucket, accepted-model download and Kaggle workflow. The account has no recorded Kaggle submission or score.
...
For this code competition, submit the successful saved notebook version through the late-submission flow when available to the signed-in account. The downloaded preview CSV is not a hidden-test score. The October 23, 2025 deadline has passed. On September 9, 2026, the signed-in account's late-submission picker displayed 100 submissions remaining today, disabled file upload, and no notebooks linked to this competition. The account's Submissions tab also showed **No submissions found**, so there is no Jigsaw submission score to report. The next submission step is to import and successfully run the canonical notebook with competition data; submission availability must be checked again at that time. Record a score only after Kaggle returns one. [Official competition](https://www.kaggle.com/competitions/jigsaw-agile-community-rules).
