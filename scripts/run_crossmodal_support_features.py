...
from scripts.crossmodal_support_features import NAMES, cross_fitted
...
CONFIG = "configs/crossmodal_support_features.json"
PRIVATE = "runs/crossmodal_support_features"
PUBLIC = "reports/crossmodal_support_features"
NOTEBOOK = "notebooks/16_crossmodal_support_investigation.ipynb"
REFERENCE = "qwen_raw"
...
SOURCES = (
    "scripts/crossmodal_support_features.py",
    "scripts/run_crossmodal_support_features.py",
    "tests/test_crossmodal_support_features.py",
    "configs/crossmodal_support_features.json",
    "docs/CROSSMODAL_SUPPORT_FEATURES.md",
...
CONTRASTS = [
    ("word_agreement", "context_evidence", "word_agreement vs context anchor"),
    ("character_agreement", "context_evidence", "character_agreement vs context anchor"),
    ("crossmodal_all", "context_evidence", "crossmodal_all vs context anchor"),
    ("alignment_null", "context_evidence", "alignment_null vs context anchor"),
    ("label_null", "context_evidence", "label_null vs context anchor"),
    ("lexical_only", "context_evidence", "lexical_only vs context anchor"),
    ("crossmodal_all", "qwen_raw", "Primary vs qwen_raw"),
    ("crossmodal_all", "frozen_basic", "Primary vs frozen_basic"),
    ("crossmodal_all", "uniform_all", "Primary vs uniform_all"),
    ("crossmodal_all", "alignment_null", "Primary vs alignment_null"),
    ("crossmodal_all", "label_null", "Primary vs label_null"),
    ("crossmodal_all", "lexical_only", "Primary vs lexical_only"),
    ("crossmodal_all", "word_agreement", "Family removal: character-agreement"),
    ("crossmodal_all", "character_agreement", "Family removal: word-agreement"),
]
PRIMARY_REFS = (
    "qwen_raw",
    "frozen_basic",
...
BANK_WIDTHS = {"aligned": 48, "alignment_null": 48, "label_null": 48, "lexical_only": 48}
...
def validate_config(spec):
    expected = {
        ...
        "hyperparameters": {
            "vocabulary_limit_per_family": 6000,
            "neighbors": 5,
            "temperature": 0.1,
        },
    }
...
def design(bundle, basic, banks, variant):
    ...
    part = slice(None)
    if variant == "word_agreement":
        part = slice(0, 24)
    elif variant == "character_agreement":
        part = slice(24, None)
...
def run_study(root, *, max_new_fits=None):
    ...
    with (
        ...
        Progress(
            work / "events.jsonl", "crossmodal_support_features_round10", heartbeat_seconds=15
        ) as log,
    ):
        ...
        if marker.exists():
            ...
            if saved["identity"] != ident:
                raise ValueError("completed Round 10 identity differs")
        ...
        result = {
            "schema": 1,
            "status": "CROSSMODAL_SUPPORT_ROUND_COMPLETE",
            "run_id": run_id,
            "identity": ident,
            "new_fits": new_fits,
            ...
            **decide(metrics, comparisons, pooled, spec),
            ...
            "limitations": [
                ...
                (
                    "Within-class alignment null preserves both marginal simila"
                    # Keep long literals split for E501.
                    "rity distributions."
                    # Keep long literals split for E501.
                ),
                (
                    "TF-IDF vocabularies and IDF are refitted on each inner ref"
                    # Keep long literals split for E501.
                    "erence pool only."
                    # Keep long literals split for E501.
                ),
                (
                    "Top-neighbor ties use normalized-text order; empty lexical"
                    # Keep long literals split for E501.
                    " vocabularies map to zeros."
                    # Keep long literals split for E501.
                ),
            ],
        }
...
def figures(result):
    ...
    rows = [
        ...
        if r["candidate"] == "crossmodal_all"
        and r["reference"] in ("alignment_null", "label_null", "lexical_only")
    ]
    ...
    rows = [
        ...
        for r in result["comparisons"]
        if r["candidate"] == "crossmodal_all"
        and r["reference"] in ("word_agreement", "character_agreement")
    ]
    ...
    for field, title in (
        ("mean_top5_agreement", "05 | Reference representation coverage"),
        ("zero_lexical_query_fraction", "06 | Missing-evidence / extrapolation checks"),
    ):
        fig = go.Figure()
        for fold, rows in outer.groupby("fold"):
            labels = rows["family"].astype(str)
            if "class" in rows:
    ...
    rows = coefs[coefs.variant.eq("crossmodal_all") & coefs.feature.isin(NAMES)]
...
def write_dashboard(root, result):
    ...
    parts = [
        (
            ...
            "charset='utf-8'><title>Lexical–semantic agreement</title>"
        ),
        "<body><h1>Jigsaw: Lexical–semantic agreement</h1>",
...
def export(root, destination=None):
    ...
    target = destination or Path.home() / "jigsaw_feature_round10_return.zip"
