"""Bounded semantic-format worker using only verified development inputs."""
...
ROOTS = ("runs/embeddings/", "runs/expanded_embeddings/", "runs/intent/", "runs/formatting/")
...
def main() -> None:
    ...
    if bucket != BUCKET or not prefix.startswith("experiments/semantic-formatting-"):
    ...
    inputs = root.parent / "development-inputs.tar.gz"
    ...
    with tarfile.open(inputs) as stream:
        ...
        for name in files:
            if name != "data/raw/train.csv" and not name.startswith(
                ("runs/released/", "runs/embeddings/", "runs/expanded/", "runs/pairs/")
            ):
                raise ValueError("Unapproved research input path")
    extract(inputs, root)
    emit("research_inputs_restored", paths=len(files), reserved_targets_present=False)
    resume = os.environ.get("JIGSAW_RESUME_PREFIX")
    if resume:
        if not resume.startswith("experiments/semantic-formatting-"):
            raise ValueError("Resume must use a project semantic-formatting checkpoint")
        ...
        for relative_root in ROOTS:
            for path in (root / relative_root).glob("*/*.json"):
                if path.name in {"contract.json", "provenance.json", "completed.json"}:
                    upload(path, prefix + "/checkpoint/" + path.relative_to(root).as_posix())
        for path in (root / "logs").glob("formatting*.jsonl"):
    ...
    status = 1
    try:
        emit("semantic_formatting_study_started")
        process = subprocess.run([python, "scripts/run_formatting.py", "--encode"], cwd=root)
        status = process.returncode
    finally:
        stop.set()
        thread.join()
        checkpoint()
        for path in (root / "reports/formatting").glob("*.json"):
            upload(path, prefix + "/public/reports/formatting/" + path.name)
        if status == 0 and not errors:
            archive = root.parent / "semantic-formatting.tar.gz"
            with tarfile.open(archive, "w:gz") as stream:
                for directory in [
                    *(root / p for p in ROOTS),
                    root / "reports/formatting",
                    root / "logs",
                ]:
            ...
            upload(archive, prefix + "/checkpoints/semantic-formatting.tar.gz")
            emit(
                "completed",
