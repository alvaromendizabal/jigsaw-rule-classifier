...
from scripts import run_crossmodal_support_features as dependency_other
from scripts import run_multiprototype_features as dependency_base
from scripts.reference_consistency_features import NAMES, cross_fitted
...
CONFIG = "configs/reference_consistency_features.json"
PRIVATE = "runs/reference_consistency_features"
PUBLIC = "reports/reference_consistency_features"
NOTEBOOK = "notebooks/18_reference_consistency_investigation.ipynb"
REFERENCE = "qwen_raw"
...
SOURCES = (
    "scripts/reference_consistency_features.py",
    "scripts/run_reference_consistency_features.py",
    "tests/test_reference_consistency_features.py",
    "configs/reference_consistency_features.json",
    "docs/REFERENCE_CONSISTENCY_FEATURES.md",
...
CONTRASTS = [
    ("agreement_features", "context_evidence", "agreement_features vs context anchor"),
    ("reciprocity_features", "context_evidence", "reciprocity_features vs context anchor"),
    ("consistency_all", "context_evidence", "consistency_all vs context anchor"),
    ("quality_null_all", "context_evidence", "quality_null_all vs context anchor"),
    ("geometry_only_all", "context_evidence", "geometry_only_all vs context anchor"),
    ("label_null_all", "context_evidence", "label_null_all vs context anchor"),
    ("consistency_all", "qwen_raw", "Primary vs qwen_raw"),
    ("consistency_all", "frozen_basic", "Primary vs frozen_basic"),
    ("consistency_all", "uniform_all", "Primary vs uniform_all"),
    ("consistency_all", "quality_null_all", "Primary vs quality_null_all"),
    ("consistency_all", "geometry_only_all", "Primary vs geometry_only_all"),
    ("consistency_all", "label_null_all", "Primary vs label_null_all"),
    ("consistency_all", "agreement_features", "Family removal: reciprocity"),
    ("consistency_all", "reciprocity_features", "Family removal: agreement"),
]
PRIMARY_REFS = (
    "qwen_raw",
    "frozen_basic",
...
BANK_WIDTHS = {"observed": 48, "quality_null": 48, "geometry_only": 48, "label_null": 48}
...
def verify_prior(root, spec):
    ...
    for module, number in ((dependency_other, 10), (dependency_base, 11)):
        ...
        if rec["run_id"] != spec[f"round{number}_run_id"]:
            raise ValueError("completed Round 10/11 identity differs")
        if rec["identity"]["environment"] != environment():
            raise ValueError("completed Round 10/11 environment differs")
...
def design(bundle, basic, banks, variant):
    ...
    mode_map = {
        "quality_null_all": "quality_null",
        "geometry_only_all": "geometry_only",
        "label_null_all": "label_null",
    }
    mode = mode_map.get(variant, "observed")
    part = slice(None)
    if variant == NEW[0]:
        part = slice(0, 24)
    elif variant == NEW[1]:
...
def run_study(root, *, max_new_fits=None):
    ...
    with (
        ...
        Progress(
            work / "events.jsonl", "reference_consistency_features_round12", heartbeat_seconds=15
        ) as log,
    ):
        ...
        if marker.exists():
            ...
            if saved["identity"] != ident:
                raise ValueError("completed Round 12 identity differs")
        ...
        result = {
            "schema": 1,
            "status": "REFERENCE_CONSISTENCY_ROUND_COMPLETE",
            "run_id": run_id,
            "identity": ident,
            "new_fits": new_fits,
            ...
            **decide(metrics, comparisons, pooled, spec),
            ...
            "limitations": [
                (
                    "Exploratory adaptive development on the same 881 "
                    # Keep this exact literal split for E501.
                    "queries, not a fresh holdout or Kaggle score."
                    # Keep this exact literal split for E501.
                ),
                (
                    "The context_evidence anchor is held fixed, selected "
                    # Keep this exact literal split for E501.
                    "after Round 9, and was never promoted."
                    # Keep this exact literal split for E501.
                ),
                (
                    "New reference features are text-group cross-fitted. "
                    # Keep this exact literal split for E501.
                    "The inherited adapted answer score is in-sample on "
                    # Keep this exact literal split for E501.
                    "support labels."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Cross-fitting the new features cannot repair that "
                    # Keep this exact literal split for E501.
                    "inherited answer-score limitation."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Simultaneous intervals cover this round, not the full "
                    # Keep this exact literal split for E501.
                    "adaptive project history."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Both new designs are fixed before either run; no "
                    # Keep this exact literal split for E501.
                    "companion-round result is read."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Neighbor disagreement is not proof of a wrong label. "
                    # Keep this exact literal split for E501.
                    "No supplied label is edited or removed."
                    # Keep this exact literal split for E501.
                ),
                (
                    "This is not Confident Learning or its "
                    # Keep this exact literal split for E501.
                    "noise-identification algorithm. The paper motivates "
                    # Keep this exact literal split for E501.
                    "label-quality awareness only."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Prior conflict dropping/soft-label experiments are "
                    # Keep this exact literal split for E501.
                    "distinct: original labels and training weights stay "
                    # Keep this exact literal split for E501.
                    "fixed."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Reliability is constructed anew inside each reference "
                    # Keep this exact literal split for E501.
                    "crossfit pool; held query labels never enter it."
                    # Keep this exact literal split for E501.
                ),
            ],
        }
...
def figures(result):
    ...
    labels = [f"Fold {v.fold} | {v['mode']} | class {v['class']}" for _, v in outer.iterrows()]
    fig = go.Figure()
    for field in ("minimum_quality", "mean_quality", "maximum_quality"):
        fig.add_bar(x=labels, y=outer[field], name=field)
    fig.update_layout(title="05 | Reference consistency weights; no relabeling", barmode="group")
    charts.append(fig)
    fig = go.Figure()
    for field in ("mean_neighbor_agreement", "mean_reciprocal_agreement"):
        fig.add_bar(x=labels, y=outer[field], name=field)
    fig.update_layout(
        title="06 | Reference-only agreement, not test-label accuracy", barmode="group"
    )
    charts.append(fig)
...
def write_dashboard(root, result):
    ...
    title = "Reference consistency and reciprocal evidence"
    parts = [
...
def export(root, destination=None):
    ...
    target = destination or Path.home() / "jigsaw_feature_round12_return.zip"
