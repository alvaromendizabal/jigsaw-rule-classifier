...
from scripts.passage_support_features import NAMES, cross_fitted
...
CONFIG = "configs/passage_support_features.json"
PRIVATE = "runs/passage_support_features"
PUBLIC = "reports/passage_support_features"
NOTEBOOK = "notebooks/19_passage_support_investigation.ipynb"
REFERENCE = "qwen_raw"
...
SOURCES = (
    "scripts/passage_support_features.py",
    "scripts/run_passage_support_features.py",
    "tests/test_passage_support_features.py",
    "configs/passage_support_features.json",
    "docs/PASSAGE_SUPPORT_FEATURES.md",
...
CONTRASTS = [
    ("word_passages", "context_evidence", "word_passages vs context anchor"),
    ("character_passages", "context_evidence", "character_passages vs context anchor"),
    ("passage_all", "context_evidence", "passage_all vs context anchor"),
    ("whole_comment_all", "context_evidence", "whole_comment_all vs context anchor"),
    ("boundary_null_all", "context_evidence", "boundary_null_all vs context anchor"),
    ("label_null_all", "context_evidence", "label_null_all vs context anchor"),
    ("passage_all", "qwen_raw", "Primary vs qwen_raw"),
    ("passage_all", "frozen_basic", "Primary vs frozen_basic"),
    ("passage_all", "uniform_all", "Primary vs uniform_all"),
    ("passage_all", "whole_comment_all", "Primary vs whole_comment_all"),
    ("passage_all", "boundary_null_all", "Primary vs boundary_null_all"),
    ("passage_all", "label_null_all", "Primary vs label_null_all"),
    ("passage_all", "word_passages", "Family removal: character_passages"),
    ("passage_all", "character_passages", "Family removal: word_passages"),
]
PRIMARY_REFS = (
    "qwen_raw",
    "frozen_basic",
...
BANK_WIDTHS = {"passages": 48, "whole_comment": 48, "boundary_null": 48, "label_null": 48}
...
def design(bundle, basic, banks, variant):
    ...
    mode_map = {
        ...
        "boundary_null_all": "boundary_null",
        "label_null_all": "label_null",
    }
    mode = mode_map.get(variant, "passages")
    part = slice(None)
    if variant == NEW[0]:
        part = slice(0, 24)
    elif variant == NEW[1]:
...
def run_study(root, *, max_new_fits=None):
    ...
    with (
        ...
        Progress(
            work / "events.jsonl", "passage_support_features_round13", heartbeat_seconds=15
        ) as log,
    ):
        ...
        if marker.exists():
            ...
            if saved["identity"] != ident:
                raise ValueError("completed Round 13 identity differs")
        ...
        result = {
            "schema": 1,
            "status": "PASSAGE_SUPPORT_ROUND_COMPLETE",
            "run_id": run_id,
            "identity": ident,
            "new_fits": new_fits,
            ...
            **decide(metrics, comparisons, pooled, spec),
            ...
            "limitations": [
                ...
                (
                    "Reference labels belong to entire reference comments, "
                    # Keep this exact literal split for E501.
                    "never automatically to their sentences."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Segmentation is a fixed punctuation/newline heuristic, "
                    # Keep this exact literal split for E501.
                    "not a parser or encoder. Max twelve chunks; excess "
                    # Keep this exact literal split for E501.
                    "chunks are merged, not discarded."
                    # Keep this exact literal split for E501.
                ),
                (
                    "Prior scope-token features and whole-comment "
                    # Keep this exact literal split for E501.
                    "lexical/semantic agreement did not establish gains; "
                    # Keep this exact literal split for E501.
                    "this tests passage localization instead."
                    # Keep this exact literal split for E501.
                ),
                (
                    "The fixed-count boundary control preserves content and "
                    # Keep this exact literal split for E501.
                    "passage count, but not individual passage lengths or "
                    # Keep this exact literal split for E501.
                    "feature norms."
                    # Keep this exact literal split for E501.
                ),
...
def figures(result):
    ...
    fig = go.Figure()
    for field in ("mean_passages", "maximum_passages"):
        fig.add_bar(x=labels, y=outer[field], name=field)
    fig.update_layout(title="05 | Observed passage coverage and bounded counts", barmode="group")
    charts.append(fig)
    fig = go.Figure()
    for field in ("multiple_passage_fraction", "zero_lexical_fraction"):
        fig.add_bar(x=labels, y=outer[field], name=field)
...
def export(root, destination=None):
    ...
    target = destination or Path.home() / "jigsaw_feature_round13_return.zip"
