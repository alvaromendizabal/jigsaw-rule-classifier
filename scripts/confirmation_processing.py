"""Bounded target-free protected-inference worker with durable shard checkpoints."""
...
def main() -> None:
    ...
    if bucket != BUCKET or not prefix.startswith("experiments/protected-confirmation-"):
    ...
    inputs = root.parent / "confirmation-inputs.tar.gz"
    input_key = os.environ["JIGSAW_INPUT_KEY"]
    if not input_key.startswith(prefix + "/inputs/"):
        raise ValueError("Inputs must belong to this owned experiment prefix")
    download(input_key, inputs)
    if sha256(inputs) != os.environ["JIGSAW_INPUT_SHA256"]:
        raise ValueError("Target-free input checksum mismatch")
    with tarfile.open(inputs) as stream:
        files = {item.name for item in stream.getmembers()}
        for name in files:
            if name != "confirmation-input-manifest.json" and (
                not name.startswith(
                    ("runs/embeddings/", "runs/model_validation/", "runs/confirmation/")
                )
                or Path(name).name == "solution.csv"
            ):
                raise ValueError("Unapproved research input path")
    extract(inputs, root)
    manifest = json.loads((root / "confirmation-input-manifest.json").read_text())
    if set(manifest["files"]) | {"confirmation-input-manifest.json"} != files:
        raise ValueError("Inference input allowlist differs")
    for name, expected_sha in manifest["files"].items():
        if sha256(root / name) != expected_sha:
            raise ValueError("Inference input member checksum differs")
    emit("target_free_inputs_restored", paths=len(files), reserved_targets_present=False)
    resume = os.environ.get("JIGSAW_RESUME_PREFIX")
    if resume:
        if not resume.startswith("experiments/protected-confirmation-"):
            raise ValueError("Resume must use a project protected-confirmation checkpoint")
    ...
    originals = {
        ...
        for p in (root / "runs").rglob("complete.json")
        if p.relative_to(root).as_posix() in files
    }
    uploaded, errors = set(), []
        ...
        for relative_root in ROOTS:
            for path in (root / relative_root).glob("*/*.json"):
                if path.name in {"contract.json", "provenance.json", "completed.json"}:
                    upload(path, prefix + "/checkpoint/" + path.relative_to(root).as_posix())
        for path in (root / "logs").glob("confirmation*.jsonl"):
    ...
    status = 1
    try:
        emit("protected_inference_started")
        run_id = os.environ["JIGSAW_RUN_ID"]
        if len(run_id) != 20 or any(c not in "0123456789abcdef" for c in run_id):
            raise ValueError("Invalid protected run identifier")
        process = subprocess.run(
            [python, "scripts/run_confirmation.py", "predict", "--run-id", run_id, "--encode"],
            cwd=root,
            timeout=6300,
        )
        ...
        thread.join()
        checkpoint()
        for path in (root / "reports/confirmation").glob("*.json"):
            upload(path, prefix + "/public/reports/confirmation/" + path.name)
        if status == 0 and not errors:
            archive = root.parent / "protected-predictions.tar.gz"
            with tarfile.open(archive, "w:gz") as stream:
                for directory in [
                    *(root / p for p in ROOTS),
                    root / "reports/confirmation",
                    root / "logs",
                ]:
                    for path in sorted(directory.rglob("*")):
                        if (
                            ...
                            and not path.name.endswith((".partial", ".lock"))
                        ):
                            stream.add(path, arcname=path.relative_to(root), recursive=False)
            upload(archive, prefix + "/checkpoints/protected-predictions.tar.gz")
            emit(
                "completed",
