...
from scripts.bm25_support_features import NAMES, cross_fitted
...
CONFIG = "configs/bm25_support_features.json"
PRIVATE = "runs/bm25_support_features"
PUBLIC = "reports/bm25_support_features"
NOTEBOOK = "notebooks/20_bm25_support_investigation.ipynb"
REFERENCE = "qwen_raw"
...
SOURCES = (
    "scripts/bm25_support_features.py",
    "scripts/run_bm25_support_features.py",
    "tests/test_bm25_support_features.py",
    "configs/bm25_support_features.json",
    "docs/BM25_SUPPORT_FEATURES.md",
...
CONTRASTS = [
    ("word_bm25", "context_evidence", "word_bm25 vs context anchor"),
    ("character_bm25", "context_evidence", "character_bm25 vs context anchor"),
    ("bm25_all", "context_evidence", "bm25_all vs context anchor"),
    ("tfidf_all", "context_evidence", "tfidf_all vs context anchor"),
    ("no_length_all", "context_evidence", "no_length_all vs context anchor"),
    ("label_null_all", "context_evidence", "label_null_all vs context anchor"),
    ("bm25_all", "qwen_raw", "Primary vs qwen_raw"),
    ("bm25_all", "frozen_basic", "Primary vs frozen_basic"),
    ("bm25_all", "uniform_all", "Primary vs uniform_all"),
    ("bm25_all", "tfidf_all", "Primary vs tfidf_all"),
    ("bm25_all", "no_length_all", "Primary vs no_length_all"),
    ("bm25_all", "label_null_all", "Primary vs label_null_all"),
    ("bm25_all", "word_bm25", "Family removal: character_bm25"),
    ("bm25_all", "character_bm25", "Family removal: word_bm25"),
]
PRIMARY_REFS = (
    "qwen_raw",
    "frozen_basic",
...
BANK_WIDTHS = {"bm25": 48, "tfidf": 48, "no_length": 48, "label_null": 48}
...
def design(bundle, basic, banks, variant):
    ...
    mode_map = {"tfidf_all": "tfidf", "no_length_all": "no_length", "label_null_all": "label_null"}
    mode = mode_map.get(variant, "bm25")
    part = (
...
def run_study(root, *, max_new_fits=None):
    ...
    with (
        ...
        Progress(
            work / "events.jsonl", "bm25_support_features_round14", heartbeat_seconds=15
        ) as log,
    ):
        ...
        if marker.exists():
            ...
            if saved["identity"] != ident:
                raise ValueError("completed Round 14 identity differs")
        ...
        result = {
            "schema": 1,
            "status": "BM25_SUPPORT_ROUND_COMPLETE",
            "run_id": run_id,
            "identity": ident,
            "new_fits": new_fits,
            ...
            **decide(metrics, comparisons, pooled, spec),
            ...
            "limitations": [
                ...
                (
                    ...
                    # Keep the literal split through formatting.
                    "Round 15 is not an input."
                    # Keep the literal split through formatting.
                ),
                (
                    "Term-frequency saturation and reference-length normalization"
                    # Keep the literal split through formatting.
                    " change existing comparisons, not labels or neural weights. "
                    # Keep the literal split through formatting.
                    "The bounded similarity is normalized for query length; it is"
                    # Keep the literal split through formatting.
                    " a BM25-inspired feature, not raw search-engine BM25. The "
                    # Keep the literal split through formatting.
                    "same count vocabularies are reused for TF-IDF and b=0 "
                    # Keep the literal split through formatting.
                    "controls."
                    # Keep the literal split through formatting.
                ),
...
def figures(result):
    ...
    fig = go.Figure()
    for field in ("mean_reference_length", "maximum_reference_length"):
        fig.add_bar(x=labels, y=outer[field], name=field)
    fig.update_layout(title="05 | Reference length and coverage", barmode="group")
    charts.append(fig)
    fig = go.Figure()
    for field in ("zero_query_fraction", "mean_similarity"):
        fig.add_bar(x=labels, y=outer[field], name=field)
...
def export(root, destination=None):
    ...
    target = destination or Path.home() / "jigsaw_feature_round14_return.zip"
