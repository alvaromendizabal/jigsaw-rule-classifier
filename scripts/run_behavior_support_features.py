...
from scripts.behavior_support_features import NAMES, cross_fitted
...
CONFIG = "configs/behavior_support_features.json"
PRIVATE = "runs/behavior_support_features"
PUBLIC = "reports/behavior_support_features"
NOTEBOOK = "notebooks/15_behavior_support_investigation.ipynb"
REFERENCE = "qwen_raw"
...
SOURCES = (
    "scripts/behavior_support_features.py",
    "scripts/run_behavior_support_features.py",
    "tests/test_behavior_support_features.py",
    "configs/behavior_support_features.json",
    "docs/BEHAVIOR_SUPPORT_FEATURES.md",
    "scripts/relational_features.py",
    "scripts/behavioral_features.py",
)
CONTRASTS = [
    ("role_evidence", "frozen_basic", "role_evidence vs basic geometry"),
    ("context_evidence", "frozen_basic", "context_evidence vs basic geometry"),
    ("conditioned_all", "frozen_basic", "conditioned_all vs basic geometry"),
    ("permuted_all", "frozen_basic", "permuted_all vs basic geometry"),
    ("uniform_all", "frozen_basic", "uniform_all vs basic geometry"),
    ("descriptor_only", "frozen_basic", "descriptor_only vs basic geometry"),
    ("conditioned_all", "qwen_raw", "Primary vs raw Qwen"),
    ("conditioned_all", "answer_only", "Primary vs answer-only"),
    ("conditioned_all", "permuted_all", "Correct behavior alignment vs reference permutation"),
    ("conditioned_all", "uniform_all", "Semantic affinity vs uniform reference weighting"),
    ("conditioned_all", "descriptor_only", "Conditional evidence beyond direct descriptors"),
    ("conditioned_all", "role_evidence", "Context evidence beyond actor roles"),
    ("conditioned_all", "context_evidence", "Actor roles beyond context evidence"),
]
PRIMARY_REFS = (
    "qwen_raw",
    "answer_only",
...
BANK_WIDTHS = {"conditioned": 48, "permuted": 48, "uniform": 48, "descriptor_only": 48}
...
def validate_config(spec):
    expected = {
        ...
        "hyperparameters": {"temperature": 0.1, "backoff": 0.2, "descriptor_columns": 12},
    }
...
def design(bundle, basic, banks, variant):
    mode = {
        "permuted_all": "permuted",
        "uniform_all": "uniform",
        "descriptor_only": "descriptor_only",
    }.get(variant, "conditioned")
    part = slice(None)
    if variant == "role_evidence":
        part = slice(0, 24)
    elif variant == "context_evidence":
        part = slice(24, None)
    ...
    return (
        ...
        ("answer/log_odds", *BASIC, *NAMES[part]),
    )
...
def run_study(root, *, max_new_fits=None):
    ...
    with (
        ...
        Progress(
            work / "events.jsonl", "behavior_support_features_round9", heartbeat_seconds=15
        ) as log,
    ):
        ...
        if marker.exists():
            ...
            if saved["identity"] != ident:
                raise ValueError("completed Round 9 identity differs")
        ...
        result = {
            "schema": 1,
            "status": "BEHAVIOR_SUPPORT_ROUND_COMPLETE",
            "run_id": run_id,
            "identity": ident,
            "new_fits": new_fits,
            ...
            **decide(metrics, comparisons, pooled, spec),
            ...
            "limitations": [
                ...
                (
                    "Behavior descriptors are approximate observable cues, never "
                    # Keep narrative chunks separate for formatting.
                    "inferred moderation labels."
                ),
                (
                    "Behavior matching includes matching absence; rare/empty "
                    # Keep narrative chunks separate for formatting.
                    "strata back off at weight 0.2."
                ),
                (
                    "Reference permutation preserves descriptor rows but breaks "
                    # Keep narrative chunks separate for formatting.
                    "their example alignment."
                ),
                (
                    "Uniform-weight control removes affinity weighting; direct-cue "
                    # Keep narrative chunks separate for formatting.
                    "control uses no labels."
                ),
            ],
        }
        ...
        atomic_json(
            public / "feature_catalog.json",
            {
                ...
                "families": [
                    "actor-conditional support evidence (24)",
                    "context-conditional support evidence (24)",
                ],
                "same_rule_only": True,
...
def figures(result):
    ...
    rows = [
        ...
        if r["candidate"] == "conditioned_all"
        and r["reference"] in ("permuted_all", "uniform_all", "descriptor_only")
    ]
    ...
    rows = [
        ...
        for r in result["comparisons"]
        if r["candidate"] == "conditioned_all"
        and r["reference"] in ("role_evidence", "context_evidence")
    ]
    ...
    for fold, rows in outer.groupby("fold"):
        fig.add_bar(
            y=rows.descriptor, x=rows.query_prevalence, orientation="h", name=f"Fold {fold}"
        )
    fig.update_layout(
        title="05 | Label-free query descriptor prevalence", barmode="group", height=700
    )
    charts.append(fig)
    fig = go.Figure()
    for fold, rows in outer.groupby("fold"):
        fig.add_bar(
            y=rows.descriptor, x=rows.zero_stratum_fraction, orientation="h", name=f"Fold {fold}"
        )
    fig.update_layout(title="06 | Empty-stratum backoff coverage", barmode="group", height=700)
    charts.append(fig)
    coefs = pd.DataFrame(result["coefficients"])
    rows = coefs[
        coefs.variant.eq("conditioned_all") & ~coefs.feature.str.startswith(("basic/", "answer/"))
    ]
    fig = go.Figure()
...
def write_dashboard(root, result):
    ...
    parts = [
        (
            ...
            # Keep narrative chunks separate for formatting.
            "charset='utf-8'><title>Behavior-conditioned support "
            # Keep narrative chunks separate for formatting.
            "evidence</title>"
        ),
        "<body><h1>Jigsaw: Behavior-conditioned support evidence</h1>",
...
def export(root, destination=None):
    ...
    target = destination or Path.home() / "jigsaw_feature_round9_return.zip"
