...
from scripts import action_window_features as feat
from scripts import run_action_window_features as run
...
def test_no_cue_uses_exact_whole_comment():
    text = "An ordinary discussion about the weather."
    actual, control, stats = feat.select_windows(text, "action")
    assert actual == control == [text]
    assert stats["cue_present"] is False


@pytest.mark.parametrize("family,cue", [("action", "should"), ("qualification", "unless")])
def test_cue_localization_preserves_matched_lengths(family, cue):
    text = " ".join(["prefix"] * 30 + [cue] + ["suffix"] * 50)
    actual, control, stats = feat.select_windows(text, family)
    assert cue in actual[0] and len(actual[0].split()) == 25
    assert [len(v.split()) for v in actual] == [len(v.split()) for v in control]
    assert stats["selected_fraction"] < 1


def test_overlapping_windows_merge():
    text = " ".join(["start"] * 20 + ["should", "recommend"] + ["end"] * 30)
    actual, _, _ = feat.select_windows(text, "action")
    assert len(actual) == 1 and "should recommend" in actual[0]


def test_far_apart_windows_keep_first_and_last():
    text = ("should " + "plain " * 30) * 12
    actual, control, stats = feat.select_windows(text, "action")
    assert len(actual) == len(control) == 6
    assert stats["spans"] == 6


def test_negation_is_not_a_label_flip():
    a, _, _ = feat.select_windows("You should not sue.", "action")
    assert a == ["You should not sue."]
    assert feat.QUALIFIER.search(a[0]) is not None


@pytest.mark.parametrize("text", ["", None, "x" * 50001])
def test_window_input_bounds(text):
    with pytest.raises(ValueError):
        feat.select_windows(text, "action")


def test_unknown_family_and_changed_budget_stop():
    with pytest.raises(ValueError):
        feat.select_windows("some text", "other")
    with pytest.raises(ValueError):
        feat.select_windows("some text", "action", radius=99)


def test_no_synthetic_span_labels():
    x, y, t, q, qt = inputs()
    banks, stats = feat.feature_block(x, y, t, q, qt)
    assert all(v["new_span_labels"] == 0 for v in stats)
    assert set(banks) == set(run.BANK_WIDTHS)


def test_query_content_does_not_fit_vocabulary():
    x, y, t, q, qt = inputs()
    _, a = feat.feature_block(x, y, t, q, qt)
    _, b = feat.feature_block(x, y, t, q, ["zzyyxx nooverlap"] * 8)
