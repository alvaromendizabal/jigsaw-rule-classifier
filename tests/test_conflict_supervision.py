...
def sample_frame():
    ...
    rows.extend(
        [
            {"row_id": row_id, "body": "duplicate conflict", "rule": "r1", "rule_violation": 0},
            {"row_id": row_id + 1, "body": "duplicate conflict", "rule": "r1", "rule_violation": 1},
            {"row_id": row_id + 2, "body": "majority conflict", "rule": "r2", "rule_violation": 1},
            {"row_id": row_id + 3, "body": "majority conflict", "rule": "r2", "rule_violation": 1},
            {"row_id": row_id + 4, "body": "majority conflict", "rule": "r2", "rule_violation": 0},
        ]
    )
...
def test_soft_keeps_empirical_target_and_weight():
    ...
    majority = out[(out.rule == "r2") & (out.body == "majority conflict")].iloc[0]
