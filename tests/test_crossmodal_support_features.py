...
from scripts import crossmodal_support_features as feat
from scripts import run_crossmodal_support_features as run
...
def test_lexical_vocabulary_is_reference_only():
    lex, width, missing = feat.lexical_cosine(
        ["known token", "known other"], ["unseenword"], "word"
    )
    assert width > 0 and np.count_nonzero(lex) == 0 and missing == 1


def test_empty_vocabulary_has_documented_zero_fallback():
    lex, width, missing = feat.lexical_cosine(["a", "!"], ["longerquery"], "word")
    assert lex.shape == (1, 2) and width == 0 and missing == 1 and not lex.any()


def test_alignment_null_preserves_marginal_lexical_evidence():
    x, y, text, q, qt = inputs()
    banks, _ = feat.feature_block(x, y, text, q, qt, seed=11)
    keep = [j for j in range(48) if j % 8 < 2]
    np.testing.assert_array_equal(banks["aligned"][:, keep], banks["alignment_null"][:, keep])
    assert not np.allclose(banks["aligned"], banks["alignment_null"])


def test_lexical_ablation_zeroes_only_crossmodal_columns():
    x, y, text, q, qt = inputs()
    bank, _ = feat.feature_block(x, y, text, q, qt)
    remove = [j for j in range(48) if j % 8 >= 2]
    keep = [j for j in range(48) if j % 8 < 2]
    assert np.count_nonzero(bank["lexical_only"][:, remove]) == 0
    np.testing.assert_array_equal(bank["aligned"][:, keep], bank["lexical_only"][:, keep])


def test_class_flip_reverses_margins():
    x, y, text, q, qt = inputs()
    a, _ = feat.feature_block(x, y, text, q, qt)
    b, _ = feat.feature_block(x, 1 - y, text, q, qt)
    for offset in (0, 24):
        np.testing.assert_allclose(
            a["aligned"][:, offset : offset + 8], b["aligned"][:, offset + 8 : offset + 16]
        )
        np.testing.assert_allclose(
            a["aligned"][:, offset + 16 : offset + 24], -b["aligned"][:, offset + 16 : offset + 24]
        )


def test_authored_aligned_neighbors_score_higher_than_misaligned():
    y = np.array([0] * 6 + [1] * 6)
    lex = np.asarray([[1, 0.9, 0.8, 0.7, 0.6, 0, 1, 0.9, 0.8, 0.7, 0.6, 0]])
    a = feat.summarize(lex, lex, y)
    b = feat.summarize(lex, lex[:, [5, 4, 3, 2, 1, 0, 11, 10, 9, 8, 7, 6]], y)
    assert a[0, 4] > b[0, 4] and a[0, 7] > b[0, 7]


def test_positive_vector_rescaling_does_not_change_features():
    x, y, text, q, qt = inputs()
    a, _ = feat.feature_block(x, y, text, q, qt)
    b, _ = feat.feature_block(3 * x, y, text, 8 * q, qt)
    for mode in a:
        np.testing.assert_allclose(a[mode], b[mode], atol=1e-12)
...
def test_zero_lexical_evidence_does_not_invent_arbitrary_semantic_neighbors():
    y = np.array([0, 0, 1, 1])
    result = feat.summarize(np.zeros((2, 4)), np.ones((2, 4)), y)
