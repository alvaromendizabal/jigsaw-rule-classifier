...
from scripts import passage_support_features as feat
from scripts import run_passage_support_features as run
...
def test_sentence_boundaries_and_tail_preserved():
    text = "First topic. You should sue! Last important advice?"
    assert feat.passages(text) == ["First topic.", "You should sue!", "Last important advice?"]
    long = " ".join(f"Clause {i}." for i in range(50))
    spans = feat.passages(long)
    assert len(spans) == 12 and "49." in spans[-1]
    assert normalize(" ".join(spans)) == normalize(long)


def test_boundary_control_preserves_count_and_content():
    text = "First short sentence. A much much longer second sentence is here."
    spans = feat.passages(text)
    control = feat.boundary_control(text, len(spans))
    assert len(control) == len(spans)
    assert normalize(" ".join(control)) == normalize(text)
    assert control != spans


def test_single_passage_matches_whole_comment_control():
    x, y, text, q, _ = inputs()
    qt = ["this query has no boundary markers number " + str(i) for i in range(8)]
    bank, _ = feat.feature_block(x, y, text, q, qt)
    np.testing.assert_array_equal(bank["passages"], bank["whole_comment"])
    np.testing.assert_array_equal(bank["passages"], bank["boundary_null"])


def test_class_flip_swaps_passage_evidence():
    x, y, text, q, qt = inputs()
    a, _ = feat.feature_block(x, y, text, q, qt)
    b, _ = feat.feature_block(x, 1 - y, text, q, qt)
    for offset in (0, 24):
        np.testing.assert_allclose(
            a["passages"][:, offset : offset + 8], b["passages"][:, offset + 8 : offset + 16]
        )
        np.testing.assert_allclose(
            a["passages"][:, offset + 16 : offset + 24],
            -b["passages"][:, offset + 16 : offset + 24],
        )


def test_cached_vector_values_do_not_affect_new_lexical_features():
    x, y, text, q, qt = inputs()
    a, _ = feat.feature_block(x, y, text, q, qt)
    b, _ = feat.feature_block(-x * 40, y, text, -q * 5, qt)
    for key in a:
        np.testing.assert_array_equal(a[key], b[key])


def test_empty_word_vocabulary_is_explicit_not_target_imputed():
    x, y, _, q, qt = inputs()
    text = ["! " * i for i in range(1, len(x) + 1)]
    bank, stats = feat.feature_block(x, y, text, q, qt)
    assert all(s["vocabulary_columns"] == 0 for s in stats if s["family"] == "word")
    np.testing.assert_array_equal(bank["passages"][:, :24], 0)


def test_aggregate_locates_strong_tail():
    value = feat.aggregate_passages(np.array([0.1, 0.1, 0.9]), [3])
    assert value[0, 0] == 0.9 and value[0, -1] == 1
    assert value[0, 6] == pytest.approx(0.8)


@pytest.mark.parametrize("text", ["", None, "x" * 50001])
def test_invalid_or_oversized_passage_text_stops(text):
    with pytest.raises(ValueError):
        feat.passages(text)


def test_vocabulary_cap_and_no_passage_labels():
    x, y, text, q, qt = inputs()
    _, stats = feat.feature_block(x, y, text, q, qt)
    assert all(s["vocabulary_columns"] <= 6000 for s in stats)
    assert all(s["new_passage_labels"] == 0 for s in stats)
    assert all(s["query_used_for_vocabulary"] is False for s in stats)
