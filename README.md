...
**Current submission notebook:** [`kaggle/submission.ipynb`](kaggle/submission.ipynb) · [download](https://github.com/alvaromendizabal/jigsaw-rule-classifier/raw/refs/heads/main/kaggle/submission.ipynb). This is the support-adapted Qwen 4B method, requiring an offline Kaggle GPU run and the verified model attachment. [Execution and submission status](reports/checkpoints/kaggle_adaptation.json).

**Historical baseline:** [`kaggle/reference.ipynb`](kaggle/reference.ipynb) preserves the lexical method whose late Version 2 scored **0.59191 public / 0.61956 private**. [Baseline receipt](reports/checkpoints/kaggle_submission.json) · [Kaggle submissions](https://www.kaggle.com/competitions/jigsaw-agile-community-rules/submissions#). [Notebook and model delivery](docs/DELIVERY.md) · [Historical model card](MODEL_CARD.md) · [Data card](DATA_CARD.md).
...
**Historical portfolio artifacts: delivered.** Five executed evidence notebooks, four authored inference examples, model/data cards and verified private S3 recovery make that scoped study reviewable and runnable. The original-training-only Kaggle notebook is preserved separately from the model trained with post-competition labels. CI verifies the historical reference’s 2,029-row training / 10-row preview workflow and replay; the new neural method has a separate real-GPU receipt. Kaggle Version 2 passed its offline preview run and its submitted hidden-test run: **0.59191 public / 0.61956 private**, recorded on September 9, 2026 as a late entry. The competition performance objective remains open.
